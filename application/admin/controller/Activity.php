...
class Activity extends Base {
     ...
     */
	 public function index(){    
       $Article =  M('activity'); 
        ...
        $referurl = !empty($url) ? $url : U('Admin/activity/index');
        ...
        if($data['act'] == 'edit'){
            $r = M('activity')->where('article_id', $data['article_id'])->save($data);
        }
        
        if($data['act'] == 'del'){
        	$r = D('activity')->where('article_id', $data['article_id'])->delete();
	    ...
	    private function initEditor()
    {
        ...
        if($article_id){
           $info = D('activity')->where('article_id', $article_id)->find();
        }
